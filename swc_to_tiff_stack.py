...
def swc_to_tiff_stack(input_path, output_path, im_size,
    ...
    for index in SWC.keys():
        ...
        result = bresenham3DLine(int(P[0] - min_x), int(P[1] - min_y), int(P[2] - min_z), int(C[0] - min_x),
                                 int(C[1] - min_y), int(C[2] - min_z))
    ...
    def filter_invalid():
        ...
        for r, c, d in zip(rr, cc, dd):
            if 0 <= r < height and 0 <= c < width and 0 <= d < depth:
                nrr.append(r)
                ncc.append(c)
                ndd.append(d)
        return nrr, ncc, ndd

    rr, cc, dd = filter_invalid()
    ...
    imageio.volwrite(output_path, image_array)
